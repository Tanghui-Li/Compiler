...
\begin{frame}{我们把重点放在三条实现线索}
    \begin{table}
    \centering
    \small
    \begin{tabular}{p{2.6cm}p{3.9cm}p{5.0cm}}
        \toprule
        \textbf{线索} & \textbf{代码落点} & \textbf{值得讲的点} \\
        \midrule
        语法覆盖 & \texttt{parser.rs, ast.rs} & 覆盖引用、数组、元组、表达式块和三类循环 \\
        语义状态 & \texttt{compiler.rs} & 作用域栈、可变性、返回类型、循环栈同时参与检查 \\
        IR 线性化 & \texttt{ir.rs, compiler.rs} & 临时变量承接表达式，标签和跳转承接结构化控制流 \\
        \bottomrule
    \end{tabular}
    \end{table}
    \vspace{0.2em}
    \centering
    源程序先变成 AST，再完成语义检查，最后生成四元式 IR。
\end{frame}

\begin{frame}{代码入口：第一份 AST 被第二份继续消费}
    \begin{columns}[T]
    \begin{column}{0.49\textwidth}
        \begin{block}{大作业 1}
            \small
            \texttt{lexer.rs} 负责字符扫描；\texttt{parser.rs} 负责递归下降；\texttt{ast.rs} 定义统一的程序结构。
        \end{block}
        \begin{itemize}
            \item 输出 Token 序列和 AST 打印。
            \item 测试按课程规则编号组织。
        \end{itemize}
    \end{column}
    \begin{column}{0.49\textwidth}
        \begin{block}{大作业 2}
            \small
            继续沿用 Lexer、Parser 和 AST，再增加 \texttt{compiler.rs} 与 \texttt{ir.rs}。
        \end{block}
        \begin{itemize}
            \item 语义错误统一进入 \texttt{errors}。
            \item 合法程序生成四元式 IR。
        \end{itemize}
    \end{column}
    \end{columns}
...
\begin{frame}{类 Rust 子集的结构覆盖}
    ...
    \begin{column}{0.46\textwidth}
        \begin{itemize}
            \item 核心难点是让这些结构进入同一棵 AST。
            \item 后续语义阶段可以继续检查类型、作用域和控制流。
            \item 所以作业 1 的关键产物是“可继续使用的 AST”。
        \end{itemize}
    \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{作业 1 产出：可继续编译的结构}
    ...
    \centering
    \small
    \begin{tabular}{p{2.7cm}p{3.9cm}p{4.9cm}}
        \toprule
...
\begin{frame}{作业 2 的转折：遍历 AST 时带着状态走}
    \begin{center}
...
\begin{frame}{语义检查：围绕 Rust 风格约束}
    ...
    \begin{tabular}{p{2.9cm}p{4.1cm}p{4.4cm}}
        ...
        引用/解引用约束 & 对非引用解引用，或写不可变引用 & \texttt{Ref/MutRef} \\
...
\begin{frame}{非基础结构：数组、引用、调用都有落点}
    \begin{table}
    \centering
    \small
    \begin{tabular}{p{2.8cm}p{4.0cm}p{4.6cm}}
        ...
        \texttt{t.0} & \texttt{TupleIndex} & 检查元组类型并取对应元素类型 \\
