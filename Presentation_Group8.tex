...
% 将 Berlin 默认的两行页脚合并为一行。
\setbeamertemplate{footline}{%
  \begin{beamercolorbox}[colsep=1.2pt]{upper separation line foot}
  \end{beamercolorbox}
  \begin{beamercolorbox}[wd=\paperwidth,ht=2.7ex,dp=1ex]{author in head/foot}
    \hspace*{.35cm}%
    \makebox[.30\paperwidth][l]{\usebeamerfont{author in head/foot}\insertshortauthor}%
    \makebox[.34\paperwidth][c]{\usebeamerfont{title in head/foot}编译原理大作业}%
    \makebox[\dimexpr.36\paperwidth-.7cm\relax][r]{\usebeamerfont{page number in head/foot}\insertframenumber{} / \inserttotalframenumber}%
    \hspace*{.35cm}%
  \end{beamercolorbox}%
}
...
\begin{frame}{词法扫描决策表}
    ...
    \begin{tabular}{p{2.6cm}p{4.2cm}p{4.5cm}}
        \toprule
        \textbf{首字符/模式} & \textbf{扫描动作} & \textbf{Token 结果} \\
        \midrule
        字母或 \texttt{\_} & 连续读入字母、数字、下划线，再查关键字表 & 关键字或 \texttt{Ident(String)} \\
        数字 & 连续读入数字并转为整数 & \texttt{IntLiteral(i64)} \\
        \texttt{= ! < > - .} & 前瞻一个字符，区分单字符和双字符符号 & \texttt{= == != <= >= -> . ..} \\
        \texttt{/} & 前瞻判断注释或除号 & 跳过注释或生成 \texttt{Slash} \\
        空白符 & 更新行列号，不生成 Token & 无 \\
        其他字符 & 生成词法错误，带行列号 & 错误信息 \\
        \bottomrule
    \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{Parser 调用层次}
    \begin{center}
    \resizebox{0.95\linewidth}{!}{
    \begin{tikzpicture}[
        node distance=0.75cm,
        box/.style={rectangle, rounded corners, draw=mainblue, thick, minimum width=3.0cm, minimum height=0.65cm, align=center, fill=softblue},
        arr/.style={-{Latex[length=2.5mm]}, thick, draw=mainblue}
    ]
        \node[box] (program) {\texttt{parse\_program}};
        \node[box, below=of program] (decl) {\texttt{parse\_function\_decl}};
        \node[box, below left=0.65cm and 1.8cm of decl] (block) {\texttt{parse\_block}};
        \node[box, below right=0.65cm and 1.8cm of decl] (type) {\texttt{parse\_type}};
        \node[box, below=of block] (stmt) {\texttt{parse\_statement}};
        \node[box, below=of stmt] (expr) {\texttt{parse\_expression}};
        \node[box, below left=0.65cm and 1.0cm of expr] (cmp) {比较/加减/乘除};
        \node[box, below right=0.65cm and 1.0cm of expr] (post) {调用/索引/字段};
    ...
    \vspace{-0.2em}
    \scriptsize
    结构解析和表达式解析分层：语句层负责控制流和声明，表达式层负责优先级和操作数结合。
\end{frame}
...
\begin{frame}{AST 节点分类}
    \begin{table}
    \centering
    \small
    \begin{tabular}{p{2.6cm}p{4.0cm}p{4.9cm}}
        \toprule
        \textbf{节点族} & \textbf{典型节点} & \textbf{保留的信息} \\
        \midrule
        顶层结构 & \texttt{Program, Declaration} & 函数声明列表 \\
        函数结构 & \texttt{FunctionDecl, Param, Block} & 函数名、参数、返回类型、语句块 \\
        语句 & \texttt{VarDecl, Assign, If, While, Return} & 控制流、声明、赋值和返回 \\
        表达式 & \texttt{BinaryOp, Call, Index, Ref, Deref} & 操作符、操作数、调用参数、访问路径 \\
        类型 & \texttt{I32, Array, Tuple, Ref, MutRef} & 语义分析需要的静态类型 \\
        \bottomrule
    \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{语义分析状态表}
    ...
    \begin{tabular}{p{3.2cm}p{4.2cm}p{4.2cm}}
        \toprule
        \textbf{状态} & \textbf{类型} & \textbf{用途} \\
        \midrule
        \texttt{scopes} & \texttt{Vec<HashMap<...>>} & 变量声明、类型、可变性和作用域 \\
        \texttt{current\_}\\\texttt{return\_type} & \texttt{Option<Type>} & 检查 \texttt{return} 是否匹配函数签名 \\
        \texttt{loop\_stack} & 标签栈 & 判断 \texttt{break/continue} 合法性并生成跳转目标 \\
        \texttt{errors} & \texttt{Vec<String>} & 汇总语义错误，避免只报第一个错误 \\
        \texttt{temp\_count}\\\texttt{label\_count} & 计数器 & 分配临时变量和控制流标签 \\
        \bottomrule
    \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{Statement 到 IR 的映射}
    \begin{table}
    \centering
    \small
    \begin{tabular}{p{2.8cm}p{4.0cm}p{4.7cm}}
        \toprule
        \textbf{AST 语句} & \textbf{生成逻辑} & \textbf{主要 IR} \\
        \midrule
        变量声明赋值 & 先翻译右值，再声明变量并赋值 & \texttt{ASSIGN} \\
        普通赋值 & 检查左值可写，再翻译右值 & \texttt{ASSIGN / DEREF\_ASSIGN} \\
        返回语句 & 检查返回类型，发射返回指令 & \texttt{RETURN} \\
        \texttt{if/else} & 条件失败跳转到 else，then 结束跳到 end & \texttt{JUMP\_IF\_FALSE, JUMP, LABEL} \\
        \texttt{while/loop} & 建立起止标签，循环体后跳回开始 & \texttt{LABEL, JUMP, JUMP\_IF\_FALSE} \\
        \bottomrule
    \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{测试矩阵}
    ...
    \begin{tabular}{p{3.0cm}p{4.2cm}p{4.5cm}}
        \toprule
        \textbf{测试文件} & \textbf{覆盖内容} & \textbf{观察输出} \\
        \midrule
        \texttt{test\_all.rs.txt} & 函数、声明、表达式、分支、循环、数组、元组 & Token、AST、IR 序列 \\
        \texttt{test\_errors}\\\texttt{.rs.txt} & 词法/语法错误 & 带行列号的错误信息 \\
        \texttt{test\_semantic}\\\texttt{\_errors.rs.txt} & 未声明变量、类型不匹配、不可变赋值、非法控制流 & \texttt{[Semantic Error]} 列表 \\
        \texttt{cargo check} & Rust 工程可编译性 & 两个工程均无编译错误 \\
        \bottomrule
    \end{tabular}
    \end{table}
\end{frame}
