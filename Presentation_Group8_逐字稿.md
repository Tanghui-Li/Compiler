...
【翻到第 2 页，主线，约 20 秒】  
两份作业串成一条前端流程。大作业 1 把源程序转换为 Token 和 AST；大作业 2 继续消费 AST，维护作用域、类型和循环状态，完成语义检查，并生成四元式 IR。

【翻到第 3 页，实现线索，约 15 秒】  
我们重点讲三条线索：语法覆盖、语义状态和 IR 线性化。表达式用临时变量承接，控制流用标签和跳转承接。

【翻到第 4 页，代码入口，约 15 秒】  
大作业 1 的核心文件是 `lexer.rs`、`parser.rs` 和 `ast.rs`。大作业 2 沿用这套结构，再增加 `compiler.rs` 做语义分析，`ir.rs` 定义四元式。

【翻到第 5 页，类 Rust 子集，约 20 秒】  
作业 1 支持的结构包括引用、可变引用、数组、元组、区间表达式、表达式块，以及 `if`、`while`、`for`、`loop`。这些结构进入同一棵 AST，给后续阶段使用。
...
【翻到第 11 页，检查点，约 25 秒】  
语义检查覆盖几类约束：变量使用前必须声明；赋值左侧必须可写；赋值、二元运算和返回语句检查类型一致；引用和解引用检查类型形态；`break`、`continue` 通过 `loop_stack` 确认在循环体内。
...
IR 使用四元式表示，每条指令包含 `op`、`arg1`、`arg2` 和 `result`。`Arg` 可以是空值、整数、变量、临时变量或标签。
...
测试分成功能集和诊断集。`test_all.rs.txt` 覆盖主要语法结构，`test_errors.rs.txt` 覆盖词法和语法错误，`test_semantic_errors.rs.txt` 覆盖语义错误。
