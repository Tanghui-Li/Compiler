        ...
        match self.peek().clone() {
            ...
            TokenKind::Let => {
                ...
                if self.at(&TokenKind::Assign) {
                    ...
                    // 类型一致性检查
                    if let Some(expected_ty) = &var_decl.ty {
                        let actual_ty = self.get_expr_type(&expr);
                        if expected_ty != &actual_ty {
                            let tok = self.peek_token().clone();
                            return Err(format!("{}:{}: 类型不匹配, 期望 '{}', 实际为 '{}'", tok.line, tok.col, expected_ty, actual_ty));
                        }
                    }
            ...
            TokenKind::For => {
                ...
                // Iterability check
                let iter_ty = self.get_expr_type(&iter_expr);
                let ok = matches!(iter_expr, Expr::Range(..)) || matches!(iter_ty, Type::Array(..));
                if !ok {
                    return Err(format!("{}:{}: 表达式不可迭代", tok.line, tok.col));
                }
            ...
            _ => {
                ...
                if self.at(&TokenKind::Assign) {
                    ...
                    if !self.is_mutable_lvalue(&expr) {
                        return Err(format!("{}:{}: 不能给不可变左值进行赋值", tok.line, tok.col));
                    }
                    
                    let rhs = self.parse_expression()?;
                    
                    // 类型一致性检查
                    let lhs_ty = self.get_expr_type(&expr);
                    let rhs_ty = self.get_expr_type(&rhs);
                    if lhs_ty != rhs_ty {
                        return Err(format!("{}:{}: 赋值类型不匹配, 左侧为 '{}', 右侧为 '{}'", tok.line, tok.col, lhs_ty, rhs_ty));
                    }
